...
const Hero = () => {
  ...
  // This callback increments our `loadedVideos` count
  const handleVideoLoad = () => {
    setLoadedVideos((prev) => prev + 1);
  };

  // Return the correct file path for a given index
  const getVideoSrc = (index) => `videos/hero-${index}.mp4`;

  // This calculates the next upcoming video index
  ...
  // Once all 4 are loaded => hide the loading screen
  useEffect(() => {
  ...
  useGSAP(() => {
    if (hasClicked) {
      ...
      gsap.to('#next-video', {
        ...
        ease: 'power1.inOut',
        onStart: () => {
          // Start playing the newly revealed video
  ...
  return (
    <div className="relative h-dvh w-screen overflow-x-hidden">
      ...
      {/* 
        HIDDEN PRELOAD AREA: This ensures all 4 videos fire onLoadedData
        and increments `loadedVideos`. 
      */}
      <div style={{ position: 'absolute', top: '-9999px', opacity: 0 }}>
        {[1, 2, 3, 4].map((idx) => (
          <video
            key={idx}
            src={getVideoSrc(idx)}
